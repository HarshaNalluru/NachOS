...
ExceptionHandler(ExceptionType which)
{
  ...
  DEBUG('a', "Shutdown, initiated by user program.\n");
    interrupt->Halt();
      ...
      // currentThread->space->~ProcessAddrSpace();
       ...
       child->space = new ProcessAddrSpace (currentThread->space,child->GetPID(),child);  // Duplicates the address space
    ...
    // ----------------- Shared memory allocation ---------------------------
      ...
      unsigned size = (unsigned) machine->ReadRegister(4);
      // creating address space 
      //printf("-------------------------------------\n");
      int extra_PagesInVM = divRoundUp(size, PageSize); // pages to be added to table
      int numpagesinVM = currentThread->space->GetNumPages(); // current num of pages 
      TranslationEntry *oldpagetable = currentThread->space->GetPageTable(); // current page table 
      TranslationEntry *newpageTable = new TranslationEntry[extra_PagesInVM+numpagesinVM];

      stats->numPageFaults = stats->numPageFaults + extra_PagesInVM;
      ...
      for (int i = 0; i < numpagesinVM; ++i){
        ...
        newpageTable[i].support=oldpagetable[i].support;
      }

      for (int i = 0; i < extra_PagesInVM; ++i){
        newpageTable[i + numpagesinVM].virtualPage = i + numpagesinVM;
        newpageTable[i + numpagesinVM].physicalPage = i + numPagesAllocated;
        newpageTable[i + numpagesinVM].valid =TRUE;
        newpageTable[i + numpagesinVM].use = FALSE;
        newpageTable[i + numpagesinVM].dirty = FALSE;
        newpageTable[i + numpagesinVM].readOnly =FALSE;
        newpageTable[i + numpagesinVM].shared =TRUE;
        newpageTable[i + numpagesinVM].support =FALSE;
        pgPID[newpageTable[i + numpagesinVM].physicalPage] = currentThread->GetPID();  // strings attached to physical pages
        pgOwner[newpageTable[i + numpagesinVM].physicalPage] = currentThread;
        pgShared[newpageTable[i + numpagesinVM].physicalPage] = TRUE;
        pgVPN[newpageTable[i + numpagesinVM].physicalPage] = i;
      }
      delete oldpagetable;
      currentThread->space->SetPageTable(newpageTable,numpagesinVM + extra_PagesInVM);
      numPagesAllocated = numPagesAllocated + extra_PagesInVM;
      currentThread->space->RestoreStateOnSwitch();

      machine->WriteRegister(2, numpagesinVM*PageSize);
      // Advance program counters.
       machine->WriteRegister(PrevPCReg, machine->ReadRegister(PCReg));
       machine->WriteRegister(PCReg, machine->ReadRegister(NextPCReg));
       machine->WriteRegister(NextPCReg, machine->ReadRegister(NextPCReg)+4);
    ...
    //------------------Page Fault Exception-----------
      ...
      //printf("reacdhed here-----------------------------\n");
      IntStatus oldLevel = interrupt->SetLevel(IntOff);  // disable interrupts

      //printf("page fault------------------\n");
      
      unsigned badVAdr = machine->registers[BadVAddrReg];
      currentThread->space->DemandPageAllocation(badVAdr);
