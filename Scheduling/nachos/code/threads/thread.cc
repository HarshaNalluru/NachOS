...
NachOSThread::Exit (bool terminateSim, int exitcode)
{
    ...
    if (status == RUNNING){
       ...
       if (stats->totalTicks - burstStartTime > 0){
        numBursts++;
        totalBurstTime = totalBurstTime + (stats->totalTicks - burstStartTime);
        g_avgcpuburst += stats->totalTicks - burstStartTime;
      if (g_mincpuburst > stats->totalTicks - burstStartTime) {
            g_mincpuburst = stats->totalTicks - burstStartTime ;
        }
        if (g_maxcpuburst < stats->totalTicks - burstStartTime) {
            g_maxcpuburst = stats->totalTicks - burstStartTime ;
        }
        cpuDelayTime = cpuDelayTime + (stats->totalTicks - burstStartTime);
        g_varcpuburst += abs(burstEstimateTime-(stats->totalTicks - burstStartTime));
      burstEstimateTime = 0.5*burstEstimateTime + 0.5*(stats->totalTicks - burstStartTime);

      }
     
    //   if (GetPID() !=0 && (stats->totalTicks - burstStartTime)!=0)
    // {
    ...
    g_totalTime +=  totalBurstTime;
    g_totalWait += total_waitTime;
    g_totalBursts += numBursts;
    burstAverage = totalBurstTime/numBursts;
    running = false;
    if (GetPID() !=0)
    {
      
      g_squarethreadcom += (stats->totalTicks)*(stats->totalTicks);
      
      g_avgthreadcom += stats->totalTicks;
      if (g_minthreadcom > stats->totalTicks) {
            g_minthreadcom = stats->totalTicks ;

        }
        if (g_maxthreadcom < stats->totalTicks) {
            g_maxthreadcom = stats->totalTicks;
        }
    }
    
    printf("\n-----%s CPU burst : %lld------\n", name, totalBurstTime);
    printf("------total %ld totalBursts %ld totalAverageBurstLength %ld------\n", g_totalTime, g_totalBursts, g_totalTime/g_totalBursts);
    printf("------code: %s numberOfBursts %d burstAverage %d------\n", name, numBursts, burstAverage);
    //printf("---------- stats  %lf \n", (double)stats->totalTicks );
   //printf("---------- x2  %lf \n",(double)g_squarethreadcom );
    counter++;
    if(currentThread->GetPID()==0){
      printf("No Wait At Main Thread\n");
    }
    else{
     
      printf("--------WaitHere : %d totalWaitTime %d-----\n", total_waitTime, g_totalWait);  
      if (counter==11){
        //FILE * f = fopen("output.txt","a");
        printf("\n-------- Min cpuburst : %d , Max cpuburst : %d Var cpuburst : %lf----------\n", g_mincpuburst,g_maxcpuburst, (double)((double)g_varcpuburst)/(g_totalTime));
        printf("\n-------- Min Completiontime : %d , Max Completiontime : %d ----------\n", g_minthreadcom,g_maxthreadcom );
        printf("\n -------- VAriance thread completion time %lf -------------- %lf \n ",(double)((double)(g_squarethreadcom)/((thread_index-1)*1.0)) ,(double)((double)(g_avgthreadcom)/(thread_index-1)*1.0)*((g_avgthreadcom)/(thread_index-1)*1.0));
        printf("\n--------Avg cpuburst :%lf , Avg threadcompletion time :%lf------\n",(double)((double)g_avgcpuburst)/(g_totalBursts) , (double)(((double)g_avgthreadcom)/(thread_index-1)) );
        printf("\n--------CPU-UTILIZATION : %lfpercent,  averageWait: %lf------\n", (double)(((double)g_totalTime)*100/stats->totalTicks) , (double)(((double)g_totalWait)/(thread_index-1)));
        //fprintf(f,"\n  %d --------CPU-UTILIZATION : %lfpercent,  averageWait: %lf------\n",algorithm, (double)(((double)g_totalTime)*100/stats->totalTicks) , (double)(((double)g_totalWait)/(thread_index-1)));
        //fclose(f);
      }
    }
...
NachOSThread::PutThreadToSleep ()
{
    ...
    // printf("\nsleep before update readyList count in list %d %d\n", scheduler->readyList->count, currentThread->GetPID());
    scheduler->UpdateThreadPriorities();
