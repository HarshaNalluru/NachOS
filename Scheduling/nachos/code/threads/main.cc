...
main(int argc, char **argv)
{
	...
	argCount = 1;
		...
		while(!feof(f)){
			exfile = new char[100];
			fscanf(f,"%s %d",exfile,&priority);
			//printf("%s %d\n",exfile,priority);
		    OpenFile *executable = fileSystem->Open(exfile);
		    ...
		    newThread = new NachOSThread(exfile);  
		    newThread->space = space;
		    if (priority != -37){
		    	newThread->nice = priority;
		    }
		    else{
		    	newThread->nice = 100;
		 	}
		    newThread->priority = newThread->basePriority + newThread->nice;
		    printf("%d\n", newThread->nice);
		    newThread->SaveUserState();
			newThread->ThreadFork(setChildContext,0);		
			priority = -37;
			//break;
