...
TimerInterruptHandler(int dummy)
{
    ...
    int is_sleep_time_over;
    IntStatus oldLevel = interrupt->SetLevel(IntOff);
    NachOSThread *temp;
    while(!(sleepList->IsEmpty())){
        temp = (NachOSThread *)sleepList->SortedRemove(&is_sleep_time_over);
        if (is_sleep_time_over <= stats->totalTicks){
            scheduler->ThreadIsReadyToRun(temp);
        }
        else{
            sleepList->SortedInsert(temp,is_sleep_time_over);
            break;
        }
    }
