...
// first, set up the translation 
    ...
    NachOSpageTable[i].physicalPage = temp; //-------------------------------------
...
// then, copy in the code and data segments into memory
    if (noffH.code.size > 0) {
        ...
        executable->ReadAt(&(machine->mainMemory[noffH.code.virtualAddr + PhysPagesComp*PageSize]),
            noffH.code.size, noffH.code.inFileAddr);
    }
    if (noffH.initData.size > 0) {
        DEBUG('a', "Initializing data segment, at 0x%x, size %d\n", 
            noffH.initData.virtualAddr, noffH.initData.size);
        executable->ReadAt(&(machine->mainMemory[noffH.initData.virtualAddr + PhysPagesComp*PageSize]),
...
{
    unsigned int i;
    NachOSThread *old_one = currentThread;
    size = (old_one->space)->size;
    ...
    for (i = 0; i < numPagesInVM; i++) {
    NachOSpageTable[i].virtualPage = i; // for now, virtual page # = phys page #
    NachOSpageTable[i].physicalPage = PhysPagesComp;
    NachOSpageTable[i].valid = old_one->space->NachOSpageTable[i].valid;
    NachOSpageTable[i].use = old_one->space->NachOSpageTable[i].use;
    NachOSpageTable[i].dirty = old_one->space->NachOSpageTable[i].dirty;
    NachOSpageTable[i].readOnly = old_one->space->NachOSpageTable[i].readOnly;  // if the code segment was entirely on 
                    // a separate page, we could set its 
                    // pages to be read-only
    PhysPagesComp++;
    }
    int old_start = (old_one->space->NachOSpageTable[0].physicalPage)*PageSize;
    int new_start = (NachOSpageTable[0].physicalPage)*PageSize;
    i = 0;
    while(i<size){
        machine->mainMemory[new_start] = machine->mainMemory[old_start];
        i++;
        old_start++;
        new_start++;
    }
